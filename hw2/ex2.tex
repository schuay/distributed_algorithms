...
\item adding $2^\ell$ to $i$ ($j$) does not affect the $\ell$ least significant
      bits of $i$ ($j$), that \label{item:a}
\item in any sequence of $2^\ell$ consecutive numbers, the $\ell$ least significant
      bits are unique, and \label{item:b}
\item that when the $\ell$ most significant bits of two numbers $x, y$ differ, 
      their ordering is fixed regardless their remaining least significant bits
      (if the most significant differing bit is $2^i$, the difference between both
      numbers is greater than the largest difference their $i$ least significant
      bits can produce ($2^i-1$)). \label{item:c}
\end{enumerate}
...
and it follows from \ref{item:b} and \ref{item:c} that they completely determine the ordering relation
of all elements in segments $S, S'$.
\end{proof}
...
(proof in the book and lecture slides).
...
is given as a counterexample. Initially, all $p_i$ with input $0$ send $msg_0$ to their
left neighbor.
In rounds $1, \ldots, n$, processors with input $1$ simply forward all received
messages, while $p_i$ with input $0$ swallow all messages. In round $n + 1$,
each processor terminates with result ``false'' if it has received a message, and
result ``true'' otherwise.

This algorithm elects a leader with $O(n)$ message complexity since a message
passes each edge at most once.
\end{proof}
...
for processor $p_{i-1}$; since the execution is admissible, $id_0$ is eventually
delivered and processed at $p_{i}$. Since $id_0$ is maximal over all IDs, it must be forwarded by $p_i$
and is therefore sent to $p_{i+1}$.

% TODO: Kinda weird.
If $i = 0$, i.e. the message has completed
the cycle and reached its sender, the statement holds trivially since it is already
satisfied for $p_0$.
\end{proof}
